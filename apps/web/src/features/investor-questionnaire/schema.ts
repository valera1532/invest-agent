...
export const investorQuestionnaireSchema = z.object({
  ...
  monthlyContribution: z.number().int().min(0).max(1_000_000),
  aiReviewFrequency: z.enum(["daily", "weekly", "monthly"]),
});

export type InvestorQuestionnaireSchema = z.infer<
  typeof investorQuestionnaireSchema
>;

export type InvestorQuestionnaireResult = InvestorQuestionnaireSchema & {
